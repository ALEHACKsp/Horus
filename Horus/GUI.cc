...
GUI::GUI() noexcept
{
    ...
    if (PWSTR pathToFonts; SUCCEEDED(SHGetKnownFolderPath(FOLDERID_Fonts, 0, nullptr, &pathToFonts)))
    {
        ...
        if (!io.Fonts->AddFontFromFileTTF((path / "tahoma.ttf").string().c_str(), 15.0f, &cfg, Helpers::getFontGlyphRanges()))
...
{
    if (ImGui::IsKeyPressedMap(ImGuiKey_Insert, false))
    {
        open = !open;
        if (!open)
...
void GUI::renderConfigWindow() noexcept
{
    ...
    ImGui::Checkbox("Cargo incemental", &incrementalLoad);
    ...
    if (ImGui::ListBox(
            "", &currentConfig, [](void *data, int idx, const char **out_text)
            {
    ...
    ImGui::PushID(0);
    if (ImGui::InputTextWithHint("", "Nombre de la config", &buffer, ImGuiInputTextFlags_EnterReturnsTrue))
    {
        if (currentConfig != -1)
    ...
    if (ImGui::Button("Abrir directorio"))
...
void GUI::renderGuiStyle2() noexcept
{
    ...
    ImGui::Begin("Horus Inventory", nullptr, windowFlags);
    ...
    if (ImGui::BeginTabBar("TabBar", ImGuiTabBarFlags_Reorderable | ImGuiTabBarFlags_FittingPolicyScroll | ImGuiTabBarFlags_NoTooltip))
    {
        ...
        if (ImGui::BeginTabItem("Configuraciones"))
        {
            renderConfigWindow();
            ImGui::EndTabItem();
        }
