...
private:
    ...
    {
        if (const auto it = std::ranges::find_if(StaticData::gameItems(), [graffitiID = StaticData::paintKits()[sealedGraffiti.get().dataIndex].id](const auto &item)
                                                 { return item.isGraffiti() && StaticData::paintKits()[item.dataIndex].id == graffitiID; });
            it != StaticData::gameItems().end())
        {
...
{
    if (ImGui::MenuItem("Inventario"))
    {
        windowOpen = true;
        ImGui::SetWindowFocus("Inventario");
        ImGui::SetWindowPos("Inventario", {100.0f, 100.0f});
    }
}
...
void InventoryChanger::drawGUI(bool contentOnly) noexcept
{
    ...
    if (!isInAddMode && ImGui::Button("A�adir items.."))
        isInAddMode = true;

    if (!isInAddMode)
    {
        ImGui::SameLine();
        if (ImGui::Button("Forzar Update"))
            InventoryChanger::scheduleHudUpdate();
    }

    constexpr auto rarityColor = [](int rarity) noexcept
    {
        constexpr auto rarityColors = std::to_array<ImU32>({IM_COL32(0, 0, 0, 0),
    ...
    if (isInAddMode)
    {
        ...
        if (ImGui::Button(("A�adir seleccionado (" + std::to_string(selectedToAdd.size()) + ")").c_str()))
        {
            isInAddMode = false;
        ...
        ImGui::InputTextWithHint("##search", "Buscar skins, pegatinas, cuchillos, kits de m�sica...", &filter);
...
void InventoryChanger::fromJson(const json &j) noexcept
{
    ...
    for (std::size_t i = 0; i < items.size(); ++i)
    {
        ...
        if (type == "Sticker")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [stickerID](const auto &gameItem)
                                                         { return gameItem.isSticker() && StaticData::paintKits()[gameItem.dataIndex].id == stickerID; });
        ...
        else if (type == "Skin")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [paintKit, weaponID](const auto &gameItem)
                                                         { return gameItem.isSkin() && StaticData::paintKits()[gameItem.dataIndex].id == paintKit && gameItem.weaponID == weaponID; });
            if (staticData == StaticData::gameItems().end())
                continue;
        ...
        else if (type == "Glove")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [paintKit, weaponID](const auto &gameItem)
                                                         { return gameItem.isGlove() && StaticData::paintKits()[gameItem.dataIndex].id == paintKit && gameItem.weaponID == weaponID; });
            if (staticData == StaticData::gameItems().end())
                continue;
        ...
        else if (type == "Music")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [musicID](const auto &gameItem)
                                                         { return gameItem.isMusic() && StaticData::paintKits()[gameItem.dataIndex].id == musicID; });
        ...
        else if (type == "Collectible")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [weaponID, isOriginal](const auto &gameItem)
                                                         { return gameItem.isCollectible() && gameItem.weaponID == weaponID && StaticData::collectibles()[gameItem.dataIndex].isOriginal == isOriginal; });
        ...
        {
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [](const auto &gameItem)
                                                         { return gameItem.isNameTag(); });
            if (staticData != StaticData::gameItems().end())
                Inventory::addItem(std::ranges::distance(StaticData::gameItems().begin(), staticData), Inventory::INVALID_DYNAMIC_DATA_IDX, false);
        }
        else if (type == "Patch")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [patchID](const auto &gameItem)
                                                         { return gameItem.isPatch() && StaticData::paintKits()[gameItem.dataIndex].id == patchID; });
            if (staticData != StaticData::gameItems().end())
                Inventory::addItem(std::ranges::distance(StaticData::gameItems().begin(), staticData), Inventory::INVALID_DYNAMIC_DATA_IDX, false);
        }
        else if (type == "Graffiti")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [graffitiID](const auto &gameItem)
                                                         { return gameItem.isGraffiti() && StaticData::paintKits()[gameItem.dataIndex].id == graffitiID; });
            if (staticData != StaticData::gameItems().end())
                Inventory::addItem(std::ranges::distance(StaticData::gameItems().begin(), staticData), Inventory::INVALID_DYNAMIC_DATA_IDX, false);
        }
        else if (type == "Sealed Graffiti")
        {
            ...
            const int graffitiID = jsonItem["Graffiti ID"];
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [graffitiID](const auto &gameItem)
                                                         { return gameItem.isSealedGraffiti() && StaticData::paintKits()[gameItem.dataIndex].id == graffitiID; });
            if (staticData != StaticData::gameItems().end())
                Inventory::addItem(std::ranges::distance(StaticData::gameItems().begin(), staticData), Inventory::INVALID_DYNAMIC_DATA_IDX, false);
        }
        else if (type == "Agent")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [weaponID](const auto &gameItem)
                                                         { return gameItem.isAgent() && gameItem.weaponID == weaponID; });
            if (staticData == StaticData::gameItems().end())
                continue;
        ...
        else if (type == "Case")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [weaponID](const auto &gameItem)
                                                         { return gameItem.isCase() && gameItem.weaponID == weaponID; });
            if (staticData != StaticData::gameItems().end())
                Inventory::addItem(std::ranges::distance(StaticData::gameItems().begin(), staticData), Inventory::INVALID_DYNAMIC_DATA_IDX, false);
        }
        else if (type == "Case Key")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [weaponID](const auto &gameItem)
                                                         { return gameItem.isCaseKey() && gameItem.weaponID == weaponID; });
            if (staticData != StaticData::gameItems().end())
                Inventory::addItem(std::ranges::distance(StaticData::gameItems().begin(), staticData), Inventory::INVALID_DYNAMIC_DATA_IDX, false);
        }
        else if (type == "Operation Pass")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [weaponID](const auto &gameItem)
                                                         { return gameItem.isOperationPass() && gameItem.weaponID == weaponID; });
            if (staticData != StaticData::gameItems().end())
                Inventory::addItem(std::ranges::distance(StaticData::gameItems().begin(), staticData), Inventory::INVALID_DYNAMIC_DATA_IDX, false);
        }
        else if (type == "StatTrak Swap Tool")
        {
            ...
            const auto staticData = std::ranges::find_if(StaticData::gameItems(), [weaponID](const auto &gameItem)
                                                         { return gameItem.isStatTrakSwapTool() && gameItem.weaponID == weaponID; });
            if (staticData != StaticData::gameItems().end())
                Inventory::addItem(std::ranges::distance(StaticData::gameItems().begin(), staticData), Inventory::INVALID_DYNAMIC_DATA_IDX, false);
        }
    }
...
void InventoryChanger::clearUnusedItemIconTextures() noexcept
{
    ...
    {
        const auto oldestIcon = std::ranges::min_element(iconTextures, [](const auto &a, const auto &b)
                                                         { return a.second.lastReferencedFrame < b.second.lastReferencedFrame; });
